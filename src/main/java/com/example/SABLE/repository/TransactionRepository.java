 ...
 * - Method names follow the convention "findBy" + PropertyName. Spring parses the
 ...
 * - Return type Optional<Transaction> is used when at most one result is expected;
 *   the repository handles the query and wraps the result in Optional (empty if not found).
 *
 * @Repository marks this interface as a persistence layer component so Spring
 * creates a proxy implementation and injects it where needed (e.g. TransactionService).
 * We do not write any implementation class; Spring Data provides it at runtime.
 */
@Repository
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
     ...
     * Spring Data JPA generates: SELECT t FROM Transaction t WHERE t.transactionId = ?1
     *
